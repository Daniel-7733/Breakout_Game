...
class Ball:
    ...
    def __init__(self, screen: _Screen, paddle_turtle: Turtle) -> None:
        ...
        # external refs
        self.paddle: Turtle = paddle_turtle
    ...
    # --- wiring from Game ---
    def total_bricks(self, count: int) -> None:
        """
        Getting total number of bricks and will change the total_alive which will be used for comparing the score and
        total number of bricks in a single game.

        :param count: (int) number of bricks
        :return: None
        """
        self.total_alive = int(count)
    ...
    def _handle_game_over(self) -> bool:
        ...
        if self.score >= self.total_alive:
            self.ball.goto(0, 0)
            self.ball.clear()
            self.ball.write("You Won", align="center", font=(self.hub_font, 50, "bold"))
